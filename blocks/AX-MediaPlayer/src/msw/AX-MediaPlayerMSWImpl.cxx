...
namespace AX::Video
{
        ...
        , _format( format )
    {
        ...
        if ( SUCCEEDED ( CoCreateInstance ( CLSID_MFMediaEngineClassFactory, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS ( &factory ) ) ) )
        {
            ...
            if ( SUCCEEDED ( factory->CreateInstance ( flags, attributes.Get ( ), _mediaEngine.GetAddressOf ( ) ) ) )
            {
                ...
                IMFSourceReader *pMediaReader = nullptr;
                HRESULT hr = MFCreateSourceReaderFromURL ( SafeBSTR{ actualPath }, nullptr, &pMediaReader );
                if( SUCCEEDED ( hr ) )
                {
                    IMFMediaType *pMediaType = nullptr;
                    hr = pMediaReader->GetCurrentMediaType ( MF_SOURCE_READER_FIRST_VIDEO_STREAM, &pMediaType );
                    if( SUCCEEDED ( hr ) )
                    {
                        UINT32 pNumerator = 0;
                        UINT32 pDenominator = 0;
                        hr = MFGetAttributeRatio ( pMediaType, MF_MT_FRAME_RATE, &pNumerator, &pDenominator );
                        if( SUCCEEDED ( hr ) )
                        {
                            _framerate = ( float ) pNumerator / ( float ) pDenominator;
                        }
                        else
                        {
                            CI_LOG_W( "Could not set movie framerate." );
                        }
                        _bitrate = MFGetAttributeUINT32 ( pMediaType, MF_MT_AVG_BITRATE, 0 );
                    }
                    else
                    {
                        CI_LOG_W( "Could not get movie media type." );
                    }
                }
                else
                {
                    CI_LOG_W( "Could not create media source." );
                }
    ...
    bool MediaPlayer::Impl::Update ( )
    {
        if ( _mediaEngine && HasVideo ( ) )
        {
            ...
            {
                if ( _renderPath->ProcessFrame ( time ) )
                {
